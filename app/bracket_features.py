...
def promote_winner_to_next_stage(match):
    ...
    if bracket.tournament.tournament_format == 'single_elimination':
        ...
        # Ищем следующий этап по stage_type
        ...
        if not next_stage:
            # Если не найден по order, ищем финал по stage_type
        ...
        # Если это полуфинал и есть матч за 3 место
        if match.stage.name.lower().startswith('полуфинал'):
            third_place_stage = BracketStage.objects.filter(bracket=bracket, stage_type='third_place').first()
            if third_place_stage:
                semifinals = BracketStage.objects.filter(bracket=bracket, name__icontains='полуфинал').order_by('order')
